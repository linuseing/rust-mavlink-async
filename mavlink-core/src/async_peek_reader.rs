...
//!
//! The main type `AsyncPeekReader`does not implement [`tokio::io::AsyncRead`] itself, as there is no added benefit
//! in doing so.
//!
...
impl<R: AsyncRead + Unpin, const BUFFER_SIZE: usize> AsyncPeekReader<R, BUFFER_SIZE> {
    /// Instantiates a new [`AsyncPeekReader`], wrapping the provided [`std::io::Read`]er and using the default chunk size
    ...
    /// from the underlying [`AsyncRead`]er until it does, an error occurs or no more data can be read (EOF).
    ///
    ...
    pub async fn peek_exact(&mut self, amount: usize) -> Result<&[u8], MessageReadError> {
        let result = self.fetch(amount, false).await;
        result
    }
